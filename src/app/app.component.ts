...
export class AppComponent {
  ...
  constructor(private store: Store<any>, private dialog: MdDialog) {
    ...
    this.showLoadingOverlay$ = store.select(fromHttpRequests.getHttpRequestsActiveState)
          .switchMap(show => show
              ? of(true)
              : timer(10000).mapTo(false));

    store.select(fromErrorOverlay.getAppErrorState)
        .skip(1)
        .subscribe(err => {
            const dialogRef: MdDialogRef<ErrorOverlayComponent> =
                this.dialog.open(ErrorOverlayComponent, { data: err.appError.errorMessage });
    });
  }
}
